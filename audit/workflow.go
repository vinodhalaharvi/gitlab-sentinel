...
import (
	...
	"github.com/vinodhalaharvi/gitlab-sentinel/prompts"
...
type VendorEndpoints struct {
	...
	// GitLab — used by the gitlabscan scanner.
	GitLabBaseURL string // e.g. "https://gitlab.com/api/v4"
	GitLabToken   string // Personal Access Token (read_api + read_repository)
	GitLabProject string // project ID or "group%2Fproject" path
}
...
const (
	...
	ScannerGitLab   ScannerID = "gitlab"
)
...
func runScanners(ctx workflow.Context, in AuditInput) ([]findings.Finding, []string) {
	...
	if len(enabled) == 0 {
		...
		if in.VendorEndpoints.GitLabToken != "" && in.VendorEndpoints.GitLabProject != "" {
			enabled = append(enabled, ScannerGitLab)
		}
	}
	...
	for _, id := range enabled {
		...
		case ScannerGitLab:
			...
			f := workflow.ExecuteActivity(ctx, gitlabscan.ActivityName, gitlabscan.ScanInput{
				ProjectID:     in.VendorEndpoints.GitLabProject,
				GitLabBaseURL: in.VendorEndpoints.GitLabBaseURL,
				GitLabToken:   in.VendorEndpoints.GitLabToken,
				ScanMRDiffs:   true,
				MaxMRs:        10,
			})
			fans = append(fans, kicked{id, f})
